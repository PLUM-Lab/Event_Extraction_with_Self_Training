...
parser.add_argument('-trw', '--trigger_weight', type=float, default=0.0)
...
parser.add_argument('-row', '--role_O_weight', type=float, default=1.0)
...
def eval_aux_dev(model, config, score_model, aux_batch):
    ...
    return avg_role_scores, avg_trigger_scores
# -------------------------------------------------------------------------------------- #
...
config.role_O_weight = args.role_O_weight
...
print('entity: ',len(vocabs['entity_type']),vocabs['entity_type'])
print('trigger: ',len(vocabs['event_type']),vocabs['event_type'])
print('role: ',len(vocabs['role_type']),vocabs['role_type'])
print('relation: ',len(vocabs['relation_type']),vocabs['relation_type'])
...
aux_train_set = AuxDataset(config, gpu=use_gpu, require_len=len(train_set),train=True)
aux_dev_set = AuxDataset(config, gpu=use_gpu, require_len=len(train_set),train=False)
...
aux_dev_num = len(aux_dev_set) // config.eval_batch_size + \
    (len(aux_dev_set) % config.eval_batch_size != 0)
...
print("Use {} optimizer and {} lr rate and {} bert lr".format(config.optimizer, config.learning_rate, config.bert_learning_rate))
...
for epoch in range(config.max_epoch):
    ...
    for batch_idx, (batch, aux_batch) in enumerate(zip(DataLoader(
        ...
        if not config.further_train and epoch > config.pretrain_epoch:
    ...
    # aux dev set
    dev_role_score = 0.0
    dev_trigger_score = 0.0
    if epoch > config.pretrain_epoch:
        progress = tqdm.tqdm(total=aux_dev_num, ncols=75,
                            desc='Aux Dev {}'.format(epoch+1))
        for batch_idx, batch in enumerate(DataLoader(aux_dev_set, batch_size=config.eval_batch_size,
                                shuffle=False, collate_fn=aux_dev_set.collate_fn)):
            progress.update(1)
            role_score, trigger_score = eval_aux_dev(model, config, score_model, batch)
            dev_role_score += role_score
            dev_trigger_score += trigger_score
            progress.set_description(f"r_score {round(dev_role_score/(batch_idx+1),3)}, t_score {round(dev_trigger_score/(batch_idx+1),3)}")
        dev_role_score /= (batch_idx+1)
        dev_trigger_score /= (batch_idx+1)
    progress.close()
    ...
    if dev_role_score > best_role_score:
        best_role_score = dev_role_score
        print('Saving best role model')
        torch.save(state, best_role_model)
        best_dev_role_model = True
        save_result(dev_result_file,
    ...
    result = json.dumps(
        {'epoch': epoch+1, 'dev': dev_scores, 'test': test_scores, 'aux_dev_score':dev_role_score})

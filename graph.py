...
class Graph(object):
    ...
    def to_dict(self):
        ...
        if len(self.entity_scores)<len(self.entities):
            self.entity_scores = [0] * len(self.entities)
        entities = [[i, j, entity_itos[k], mention_itos[l], s] for (i, j, k), (_, _, l), s in zip(self.entities, self.mentions, self.entity_scores)]
        if len(self.trigger_scores)<len(self.triggers):
            self.trigger_scores = [0] * len(self.triggers)
        triggers = [[i, j, trigger_itos[k], l] for (i, j, k), l in zip(self.triggers, self.trigger_scores)]
        if len(self.relation_scores)<len(self.relations):
            self.relation_scores = [0] * len(self.relations)
        relations = [[i, j, relation_itos[k], l] for (i, j, k), l in zip(self.relations, self.relation_scores)]
        if len(self.role_scores)<len(self.roles):
            self.role_scores = [0] * len(self.roles)
    ...
    def copy(self):
        ...
        graph.graph_local_score = copy.copy(self.graph_local_score)
    ...
    def clean_role(self):
        ...
        for i in range(len(self.roles)):
            role = self.roles[i]
            role_score = self.role_scores[i]
            if not role[2] == self.vocabs['role_type']['O']:
                roles.append(role)

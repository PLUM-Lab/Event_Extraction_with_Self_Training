...
for i in range(5):
    data = []
    with open('experiment/label_data/ere/labeled_dataset_'+str(i+1)+'.txt','r') as fin:
        for line in fin:
    ...
    for inst in data:
        ...
        if inst['sent_id'] in confident_ids:
            if inst['sent_id'] in order_list:
                order_list[inst['sent_id']].append(inst['avg_score'])
            else:
                order_list[inst['sent_id']] = [inst['avg_score']]
output_list = []
...
with open('dataset/aligned_amr3.0/order_list_ere.json','w') as fout:

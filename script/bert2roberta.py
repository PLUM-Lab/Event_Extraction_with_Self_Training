"""
This is for convert bert-large to roberta-large for event extraction dataset
this code only change the token lengh where the trigger and entity spans remain untouched. 
"""
...
def transform_for_amrie(data_dict, tokenizer):
    ...
    for i, value in enumerate(tokens_offset_list):
        ...
        # if not  token == ''.join(tokenslist[span_start:span_end]):
        #     print('warning: ','original: ',token, 'new', ''.join(tokenslist[span_start:span_end]), span_start, span_end)
        
        token_lens.append(span_end-span_start)
        start_end.append((span_start,span_end))


        # test entity trigger agreement
    # if not sum(token_lens) == len(pieces):
    #     # print(tokens)
    #     # print(pieces)
    #     # print(offset_mappings)
    #     # print(start_end)
    #     # print(token_lens)
    #     # print(sum(token_lens), len(pieces))
        # return None
    ...
    for length in token_lens:
        ...
        for i in range(length):
            try:
                token += pieces[offset][1:] if pieces[offset].startswith('\u0120') else pieces[offset]
            except:
                return None
            offset+=1
        recombine_tokens.append(token)
    for ent in new_data_dict['entity_mentions']:
        start = ent['start']
        end = ent['end']
        text = ent['text']
        if not ''.join(text.split(' ')) == ''.join(recombine_tokens[start:end]):
            print('----------------------- wrong entity -------------------------------------')
            print(text)
            print(' '.join(recombine_tokens[start:end]))
            # print(tokens)
            # print(pieces)
            # print(offset_mappings)
            # print(start_end)
    ...
    for event in new_data_dict['event_mentions']:
    ...
    # for i, entity in enumerate(new_data_dict["entity_mentions"]):
    #     ent_start, ent_end = entity["start"], entity["end"]
    #     ent_text = entity["text"]
    #     print(ent_text, ''.join(tokens[ent_start:ent_end]))
    #     assert ent_text == ''.join(tokens[ent_start:ent_end])

    # for i, event in enumerate(data_dict["event_mentions"]):
    #     eve_start, eve_end = event["trigger"]["start"], event["trigger"]["end"]
    #     eve_text = event["trigger"]["text"]
    #     print(eve_text, ' '.join(tokens[eve_start:eve_end]))
    #     assert eve_text == ' '.join(tokens[eve_start:eve_end])

...
""" this eval requires the offset of triggers match in role"""
...
def convert_arguments(triggers, entities, roles):
    ...
    for trigger_idx, entity_idx, role in roles:
        ...
        tri_start, tri_end, trigger_label = triggers[trigger_idx]
        args.add((arg_start, arg_end, tri_start, tri_end, trigger_label, role))
    return args
    ...
    for gold_graph, pred_graph in zip(gold_graphs, pred_graphs):
        ...
        for pred_arg in pred_args:
            ...
            gold_idn = {item for item in gold_args
                        if item[0] == pred_arg[0] and item[1] == pred_arg[1] and item[2] == pred_arg[2] and item[3] == pred_arg[3]
                        and item[4] == pred_arg[4]}
            assert len(gold_idn) <= 1
            if gold_idn:
                arg_idn_num += 1
                gold_class = {item for item in gold_idn if item[-1] == pred_arg[-1]}
